  ...
  for (i in 1:length(planPhe)) {
    ...
    for (j in 1:nTrait) {
      ...
      if (auto_optim) {
        ...
        if (length(covariates) == 1) {  covariates <- list(covariates) }
        if (length(fixedEffects) == 1) {  fixedEffects <- list(fixedEffects) }
        if (length(randomEffects) == 1) {  randomEffects <- list(randomEffects) }
